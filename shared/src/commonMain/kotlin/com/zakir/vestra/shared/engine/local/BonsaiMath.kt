...
/**
 * Host-side math for the Bonsai Image 4B pipeline (`local-bonsai-image-v1`): the
 * FlowMatch-Euler sigma schedule, DiT position ids, seeded Gaussian noise, and the
 * VAE latent unpatchify. Kept platform-free so it is testable without a device.
 *
 * Ported from the Apache-2.0 Kotlin reference at
 * https://github.com/john-rocky/hf-to-litertlm/tree/main/bonsai_image_work/device/BonsaiAppAndroid
 * (`BonsaiMath.kt`, Daisuke Majima), itself a port of the model's Python reference
 * (`generate.py` in litert-community/Bonsai-Image-ternary-4B). Both the sigma-schedule
 * derivation and the unpatchify index mapping are documented at that model card.
 */
object BonsaiMath {
    const val SEQ = 256
    ...
    /**
     * FLUX.2-klein sigma schedule: linspace shifted by the empirical mu, exponential
     * time-shift; timestep == sigma. The model is step-distilled to 4 steps by default.
     */
    ...
    /**
     * Seeded standard-normal noise (1, 1024, 128) via SplitMix64 + Box-Muller. Deterministic
     * per seed so (prompt, seed) is reproducible, but not verified bit-exact against the
     * reference iOS/Android apps' noise stream — only that it is deterministic and
    ...
    fun noise(seed: Long): FloatArray {
        ...
        while (i < n) {
            ...
            val theta = 2.0 * kotlin.math.PI * uniform()
    ...
    /**
     * (1024, 128) packed tokens -> (1, 32, 64, 64) VAE latent: per-packed-channel BatchNorm
     * affine first, then the 2x2 patch unfold (packed channel `m = c*4 + i*2 + j` lands at
